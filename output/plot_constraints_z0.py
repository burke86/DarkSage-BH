...
#indir = '/Users/adam/DarkSage_runs/570_mm_pso3rounded/'
...
#   0 = Mini Millennium, 1 = Full Millennium, 2 = SMDPL, 3 = Genesis-Millennium, 4=Genesis-Calibration, 4=MDPL2
...
transparent = False # True for no background in plots
cline = 'k' # Colour for axes, lines and text (best set to 'k', but if doing transparent for dark slides, 'w' is recommended instead)
thickness = 1.5 # Width of axis boundaries and ticks
fsize = 30 # Font size for the figures

matplotlib.rcParams.update({'font.size': fsize, 'xtick.major.size': 10, 'ytick.major.size': 10, 'xtick.major.width': thickness, 'ytick.major.width': thickness, 'ytick.minor.size': 5, 'xtick.minor.size': 5, 'xtick.direction': 'in', 'ytick.direction': 'in', 'axes.linewidth': thickness, 'text.usetex': True, 'font.family': 'serif', 'font.serif': 'Times New Roman', 'legend.numpoints': 1, 'legend.columnspacing': 1, 'legend.fontsize': fsize-4, 'xtick.top': True, 'ytick.right': True, 'xtick.color': cline, 'ytick.color': cline, 'axes.edgecolor': cline, 'text.color': cline, 'axes.labelcolor': cline})
...
SMF_x, SMFy = r.massfunction(SM, Lbox, range=[SM_med-0.1, 12.1], ls='--', ax=ax[0], label=r'{\sc Dark Sage}, $N_{\rm p}\!\geq\!20$', c=cline, return_data=True)
np.savetxt(outdir+'SMF.txt', np.column_stack((SMF_x, SMFy)), fmt=['%.4f', '%.8e'], header='logMstar [Msun], Phi [Mpc^-3 dex^-1] (h='+str(h)+', IMF=Chabrier)')

r.massfunction(SM[G['LenMax']>=100], Lbox, range=[SM_med-0.1, 12.1], ax=ax[0], label=r'{\sc Dark Sage}, $N_{\rm p,max}\!\geq\!100$', c=cline)
...
r.stellar_massfunction_obsdata(h, ax[0], B=True, W=True)
...
r.massfunction(HIM, Lbox, range=[HIM_med-0.1, 11.5], ls='--', ax=ax[1], c=cline)
r.massfunction(HIM[G['LenMax']>=100], Lbox, range=[HIM_med-0.1, 11.5], ax=ax[1], c=cline)
...
r.massfunction(H2M, Lbox, range=[H2M_med-0.1, 11.5], ls='--', ax=ax[2], c=cline)
r.massfunction(H2M[G['LenMax']>=100], Lbox, range=[H2M_med-0.1, 11.5], ax=ax[2], c=cline)
...
r.savepng(outdir+'1-MassFunctions', xsize=1700, ysize=512, fig=fig, transparent=transparent)
##### ====================== #####
...
ax[0].plot(np.log10(x_av), np.log10(y_mean), cline+'--', lw=2, label=r'{\sc Dark Sage}, $N_{\rm p} \! \geq \! 20$')
x_av, y_high, y_med, y_low, y_mean = r.percentiles(SM[G['LenMax']>=100], (HIM/SM)[G['LenMax']>=100], bins=bins, addMean=True, Nmin=Nmin)
ax[0].plot(np.log10(x_av), np.log10(y_mean), cline+'-', lw=2, label=r'{\sc Dark Sage}, $N_{\rm p,max} \! \geq \! 100$')
ax[0].plot(logM_B15, logHIfrac_B15, 's', color='purple', ms=10, label=r'Brown et al.~(2015)', alpha=0.5, zorder=3)
ax[0].plot(np.log10(mean_SM), np.log10(mean_HIfrac), cline+'*', ms=10, label=r'{\sc Dark Sage}, matched bins', zorder=2)
...
ax[1].plot(np.log10(SM_mean), lOH_mid, cline+'--', lw=3)
ax[1].plot(np.log10(SM_mean), lOH_low, cline+'--', lw=1.5)
ax[1].plot(np.log10(SM_mean), lOH_high, cline+'--', lw=1.5)
SM_mean, lOH_high, lOH_mid, lOH_low = r.percentiles(SM[G['LenMax']>=100], lOH[G['LenMax']>=100], bins=bins, Nmin=Nmin)
ax[1].plot(np.log10(SM_mean), lOH_mid, cline+'-', lw=3, label=r'{\sc Dark Sage} median')
ax[1].plot(np.log10(SM_mean), lOH_low, cline+'-', lw=1.5)
ax[1].plot(np.log10(SM_mean), lOH_high, cline+'-', lw=1.5, label=r'16$^{\rm th}$ \& 84$^{\rm th}$ \%iles')
...
fig.subplots_adjust(hspace=0, wspace=0, left=0, bottom=0, right=1.0, top=1.0)
r.savepng(outdir+'2-HIfrac_MassMet', xsize=768, ysize=700, transparent=transparent)
##### =================================== #####
...
r.BH_bulge_obs(h)
plt.plot(np.log10(BM_mean), np.log10(BHM_mid), cline+'--', lw=3, label=r'{\sc Dark Sage} median, $N_{\rm p}\!\geq\!20$')
plt.plot(np.log10(BM_mean), np.log10(BHM_high), cline+'--', lw=1.5)
plt.plot(np.log10(BM_mean), np.log10(BHM_low), cline+'--', lw=1.5)
...
floor = 1
BHM_low[BHM_low<=floor] = floor
plt.plot(np.log10(BM_mean), np.log10(BHM_mid), cline+'-', lw=3, label=r'Median, $N_{\rm p,max}\!\geq\!100$')
plt.plot(np.log10(BM_mean), np.log10(BHM_high), cline+'-', lw=1.5)
plt.plot(np.log10(BM_mean), np.log10(BHM_low), cline+'-', lw=1.5, label=r'16$^{\rm th}$ \& 84$^{\rm th}$ \%iles')
...
r.savepng(outdir+'3-BHBM', xsize=768, ysize=400, transparent=transparent)
##### ================================ #####
...
plt.plot(np.log10(Vmax_mid), np.log10(BaryM_mean), cline+'--', lw=3, label=r'{\sc Dark Sage} median, $N_{\rm p}\!\geq\!20$')
plt.plot(np.log10(Vmax_high), np.log10(BaryM_mean), cline+'--', lw=1.5)
plt.plot(np.log10(Vmax_low), np.log10(BaryM_mean), cline+'--', lw=1.5)
filt = filt * (G['LenMax']>=100)
BaryM_mean, Vmax_high, Vmax_mid, Vmax_low, Vmax_mean  = r.percentiles(BaryM[filt], Vmax[filt],  bins=10**np.arange(BaryM_med-0.1,12,0.1), addMean=True, Nmin=Nmin)
plt.plot(np.log10(Vmax_mid), np.log10(BaryM_mean), cline+'-', lw=3, label=r'{\sc Dark Sage} median, $N_{\rm p,max}\!\geq\!100$')
plt.plot(np.log10(Vmax_high), np.log10(BaryM_mean), cline+'-', lw=1.5)
plt.plot(np.log10(Vmax_low), np.log10(BaryM_mean), cline+'-', lw=1.5, label=r'{\sc Dark Sage} 16$^{\rm th}$ \& 84$^{\rm th}$ \%iles')
#
...
r.savepng(outdir+'4-BTF', xsize=768, ysize=400, transparent=transparent)
##### ============================= #####
...
for a in args:
    ax[0].plot(R_av[a,:], Sigma_HI[a,:], cline+'.-', lw=1, alpha=0.2)
    ax[1].plot(R_av[a,:], Sigma_H2[a,:], cline+'.-', lw=1, alpha=0.2)
ax[0].plot([0,1], [0,1], cline+'.-', lw=1, alpha=0.5, label=r'{\sc Dark Sage}') # legend
...
fig.subplots_adjust(hspace=0, wspace=0, left=0, bottom=0, right=1.0, top=1.0)
r.savepng(outdir+'5-HIH2Surface', xsize=768, ysize=700, transparent=transparent)
##### ========================== #####
